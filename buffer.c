...
Buffer * b_create (short init_capacity,char inc_factor,char o_mode){
	...
	if (init_capacity < 0 || (unsigned char)inc_factor < 0 || ((o_mode != 'f' && o_mode != 'a' && o_mode != 'm'))){
		...
		if (buff->cb_head){ /* Proceed if malloc succeeded. Else, return Null */
			...
			if ( o_mode == 'f' || (unsigned char)inc_factor == 0 ){
				...
				buff->inc_factor = 0;
				/*set to fixed mode if mode o_mode is 'f' and inc_factor =! 0 */
...
int b_isfull (Buffer * const pBD){
	if (pBD){
		if (pBD->capacity - pBD->addc_offset > 0){
			return 0; /*not full*/
		} else if (pBD->capacity - pBD->addc_offset == 0){
			return 1; /*full*/
		} else {
			/*Problem with buffer */
			return R_FAIL_1;
...
Buffer *b_pack(Buffer * const pBD){
	...
	if (pBD){ /*check if buffer pointer is valid */
		...
		if (temp){ /*if temp realloc is valid */
			...
			pBD->r_flag = 1; /*Set realloc flag to 1 */
